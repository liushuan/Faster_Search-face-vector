...
static float gethamDistance(float feature1[], float feature2[], unsigned int size) {
	float distanceCos = 0;
	for (int j = 0; j < size; j++)
	{
		distanceCos += abs(feature1[j] - feature2[j]);
	}
	return distanceCos;
}
...
Faster_Search::Faster_Search(const int dim, const float threhold1, const unsigned int max_length)
	:dim(dim), threhold1(threhold1), MaxLength(max_length) {
	data_graph = std::make_shared<Data_FasterSearch>(dim);
	base = new float[dim];
	//memset(base, 1.0f, dim);
	for (size_t i = 0; i < dim; i++)
	{
		base[i] = 1;
	}
	...
	threhold2 = std::acos(threhold1);
	threhold2 = 2.0f;
	std::cout << "threhold2 is:" << threhold2 << std::endl;
};
...
void Faster_Search::build_graph() {
	...
	//int mod = (length / 100);
	search_use_dim =50;
	base_all.resize(search_use_dim);
	for (size_t i = 0; i < search_use_dim; i++)
	{
		base_all[i] = new float[dim];
		memset(base_all[i], 0.0f, dim);
	}
	
	for (size_t i = 0; i < length; i++)
	{
		//int sum = 0;
		for (size_t j = 0; j < data_graph->dim; j++)
		{
			base_all[i % search_use_dim][j] += (data_graph->face_info.datas[i %search_use_dim][j]);
			//sum += (data_graph->face_info.datas[i %search_use_dim][j]) > 0;
		}
	...
	for (unsigned int i = 0; i < search_use_dim; i++)
	{
		...
		for (unsigned int j = 0; j < length; j++)
		{
			float acos_dis = std::acos(getAngleDistance(data_graph->face_info.datas[j], base, data_graph->dim));
			cos_index.insert(std::make_pair(acos_dis, j));
...
std::string Faster_Search::search(float *test) {
	...
	for (unsigned int i = 0; i < search_use_dim; ++i)
	{
		//base[i] = -base[i];
		base = base_all[i];
		float acos_diss =  std::acos(getAngleDistance(base, test, data_graph->dim));
		
		if ((acos_diss - threhold2) < 0) {
			start = 0;
		}
		else {
			start = my_lower_bound(data_graph->base_coss[i], length, acos_diss - threhold2);
		}

		if (acos_diss + threhold2 > PI) {
			end = length;
		}
		else {
			end = my_upper_bound(data_graph->base_coss[i], length, acos_diss + threhold2) + 1;
		}
	...
	/*if (max_cos < 0.3) {
		...
		for (size_t i = 0; i < dim; i++)
		{
			sum1 += data_graph->face_info.datas[count][i];
			sum2 += test[i];

			s1 += data_graph->face_info.datas[count][i] > 0;
			s2 += test[i] > 0;

			m += abs(data_graph->face_info.datas[count][i] - test[i]);
		}
		std::cout << " " << abs(sum2 - sum1) << ":" << max_cos << ":" << m << " ";
	}*/
...
std::string Faster_Search::search2(float *test) {
	...
	for (unsigned int i = 0; i < search_use_dim; ++i)
	{
		//base[data_graph->sort_var_index[i]] = -base[data_graph->sort_var_index[i]];
		base = base_all[data_graph->sort_var_index[i]];
		float acos_diss = std::acos(getAngleDistance(base, test, data_graph->dim));
		if ((acos_diss - threhold2) < -10) {
			start = 0;
		}
		else {
			start = my_lower_bound(data_graph->base_coss[data_graph->sort_var_index[i]], length, acos_diss - threhold2);
		}
		...
		if (acos_diss + threhold2 > PI) {
			...
			end = my_upper_bound(data_graph->base_coss[data_graph->sort_var_index[i]], length, acos_diss + threhold2) + 1;
		}
		// 15ms
...
bool Faster_Search::add_one_person(float * add_data, std::string face_id) {
	...
	for (unsigned int i = 0; i < data_graph->dim; i++)
	{
		...
		float acos_dis = std::acos(getAngleDistance(add_data, base, data_graph->dim));
		//cos_index.insert(std::make_pair(dis, data_faster_search.face_info.face_ids[j]));
		unsigned int pos = get_insert_pos(data_graph->base_coss[i], length, acos_dis);
		//int pos = insert_array_data<float>(data_faster_search.base_coss[i], length, dis);
		insert_array<float>(data_graph->base_coss[i], length, acos_dis, pos);
