...
#define _CONTROL_NUM 100
	...
	void IndexNSG::Save(const char *filename ) {
		...
		std::ofstream in;
		in.open(file_text, std::ios::app);
		in << width << "\n";
		in << ep_ << "\n";
		for (int i = 0; i < nd_; i++)
		{
			unsigned GK = (unsigned)final_graph_[i].size();
			std::string dataline = std::to_string(GK);
			for (size_t j = 0; j < GK; j++)
			{
				dataline += " " + std::to_string(final_graph_[i].data()[j]);
			}
			in << dataline << "\n";
		}
		in.close();
	...
	void IndexNSG::Load(const char *filename) {
		...
		//write_final graph////////////////////
		/*std::string final_graph_files = "E:\\work_space\\source_code\\face_search\\nsg_windows\\final_graph.text";
		std::ofstream out;
