...
namespace kgraph {
    ...
    static unsigned const default_L = 120;
    static unsigned const default_K = 50;
    ...
    public:
         ...
         * @param path Path to the index file.
         */
        virtual void save (char const *path, std::vector<double> labels, int format = FORMAT_DEFAULT) const = 0; // save to file
        /// Build the index
		...
		virtual unsigned search1 (float * data, float * test, int dim)  = 0;
		virtual unsigned search2 (float * data, float * test, int dim)  = 0;
		virtual bool found_id(int i, double label) = 0;
