...
#pragma omp parallel for
    ...
    public:
        ...
        virtual void save (char const *path, std::vector<double>labels, int format) const {
          ...
          //  os.write(reinterpret_cast<char const *>(&sig_cap), sizeof(sig_cap));
            ...
            for (unsigned i = 0; i < graph.size(); ++i) {
				if (i < 5)
					std::cout << "Index:" << labels[i] << std::endl;
                auto const &knn = graph[i];
                uint32_t K = knn.size();
		...
		virtual bool KGraphImpl::found_id(int i, double label) {
			int length = graph[i].size();
			for (size_t j = 0; j < length; j++)
			{
				if (graph[i][j].id == label) {
					return true;
				}
			}
			return false;
		...
		virtual unsigned KGraphImpl::search1(float * data, float * test, int dim) {
			int length = graph.size();
			float max_cos = -1;
			unsigned start_index = -1;
			for (size_t i = 0; i < length; i++)
			{
				float dist_cos = getAngleDistance(data + i * dim, test, dim);
				if (dist_cos > max_cos) {
					max_cos = dist_cos;
					start_index = i;
				}
		...
		virtual unsigned KGraphImpl::search2(float * data, float * test, int dim) {
			int length = graph.size();
			const unsigned int K = 100;

			unsigned char * flag = new unsigned char[length];
			memset(flag, 0, length * sizeof(unsigned char));
			unsigned int queue[K];
			int start_index = 0;
			int min_index = 0;
			float max_cos = -1;
			while (true) {
				...
				{
					queue[i] = graph[start_index][i].id;
				}
				//找最近的
				...
				{
					if (flag[queue[i]] == 0) {
						float dist_cos = getAngleDistance(data + queue[i] * dim, test, dim);
						if (dist_cos > max_cos) {
							max_cos = dist_cos;
							min_index = queue[i];
						}
				...
				for (size_t i = 0; i < K; i++)
				{
					if (queue[i] != min_index) {
						for (size_t j = 0; j < K; j++)
						{
							flag[graph[queue[i]][j].id] = 1;
						}
					}
				}
